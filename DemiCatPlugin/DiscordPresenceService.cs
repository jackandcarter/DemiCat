...
public class DiscordPresenceService : IDisposable
{
    ...
    private async Task<RefreshOutcome> RefreshInternalAsync()
    {
        if (!ApiHelpers.ValidateApiBaseUrl(_config))
        {
            ...
            return RefreshOutcome.FromFailure(RefreshFailureCooldown);
        }
        ...
        try
        {
            ...
            if (response.StatusCode == (HttpStatusCode)429)
            {
                ...
                return RefreshOutcome.FromFailure(delay);
            }
            ...
            return RefreshOutcome.FromSuccess(RefreshSuccessCooldown);
        }
        catch (Exception ex)
        {
    ...
    private void FinalizeRefreshTask(Task<RefreshOutcome> task)
    {
        ...
        if (task.IsFaulted)
        {
            outcome = RefreshOutcome.FromFailure(RefreshFailureCooldown);
        }
    ...
    private readonly struct RefreshOutcome
    {
        ...
        public static RefreshOutcome FromSuccess(TimeSpan delay) => new(true, delay);
        public static RefreshOutcome FromFailure(TimeSpan delay) => new(false, delay);
    }
